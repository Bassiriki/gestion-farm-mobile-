      ...
      <form onSubmit={handleSubmit} className={`flex flex-1 flex-col gap-4 ${fullScreen ? 'px-4 pb-28 pt-4' : ''}`}>
        ...
        {cultureId !== 'none' && cultures.find(c => c.id === cultureId)?.statut === 'recolte' && (
          <div className="p-3 text-sm text-amber-700 bg-amber-50 rounded-xl border border-amber-200">
            ⚠️ Cette culture est clôturée. Vous ne pouvez pas modifier ou ajouter de dépenses pour celle-ci.
          </div>
        )}
        ...
        {fullScreen ? (
          <div className="fixed bottom-0 left-0 right-0 z-10 bg-background/90 backdrop-blur-md border-t border-border p-4">
            <button
              type="submit"
              disabled={loading || !montant || !categorie || (cultureId !== 'none' && cultures.find(c => c.id === cultureId)?.statut === 'recolte')}
